 ...
 *
 */
public class QueueDemo extends DataStructure implements ArrayLike {
	...
	public QueueDemo(int size, JPanel panel) {
	 ...
	 * amortized O(1) overall if we want the queue to be able to grow (like this one
	...
	/**
	 * We can't remove a specific item from a queue: we can only remove the first
	...
	/**
	 * Pop removes the first item and returns it. When using an ArrayList, this is
	 * O(N), since the items need to shift to make room. However, this can be O(1)
	 * if using a circular array or another underlying data structure!
	 * 
	 * Queues are most commonly implemented with Linked Lists, which we'll look at
	 * soon, because they have simple O(1) insertion/removal to the front of the
	 * list
	 * 
	 * Queues also use the terminology "dequeue" and "poll" here sometimes.
	 * 
	 * @return
	 */
	public Item pop() {
	...
	/**
	 * Sometimes we just want to observe the first item without removing it. This is
	 * always O(1)
	 * 
	 * @return
	 */
	...
	/**
	 * There is no concept of "searching" a queue, we only ever care about the first
	 * and last item.
	 */
	@Override
	...
	/**
	 * There is no concept of "sorting" a queue, we only ever care about the first
	 * and last item.
	 * 
